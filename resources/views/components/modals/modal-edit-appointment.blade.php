    ...
    <div x-show="openEdit === {{ $appointment->id }}" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" style="display: none;">
        <div class="bg-white rounded-xl shadow-xl p-8 w-full max-w-md relative">
            ...
            <form action="{{ route('appointments.update', $appointment->id) }}" method="POST">
                ...
                @method('PUT')
                <div class="mb-4">
                    <label for="edit_client_id_{{ $appointment->id }}" class="block text-sm font-medium text-gray-700">Client</label>
                ...
                <div class="mb-4">
                    <label for="edit_service_id_{{ $appointment->id }}" class="block text-sm font-medium text-gray-700">Service</label>
                ...
                <div class="mb-4">
                    <label for="edit_scheduled_at_{{ $appointment->id }}" class="block text-sm font-medium text-gray-700">Date and Time</label>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" id="edit_status_{{ $appointment->id }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                        <option value="pendente" {{ $appointment->status == 'pendente' ? 'selected' : '' }}>Pending</option>
                        <option value="confirmado" {{ $appointment->status == 'confirmado' ? 'selected' : '' }}>Confirmed</option>
                        <option value="cancelado" {{ $appointment->status == 'cancelado' ? 'selected' : '' }}>Canceled</option>
                        <option value="concluido" {{ $appointment->status == 'concluido' ? 'selected' : '' }}>Completed</option>
                    </select>
                </div>
                <div class="flex justify-end gap-2 mt-6">
                    <button type="button" @click="openEdit = null" class="px-4 py-2 bg-gray-100 text-gray-800 rounded hover:bg-gray-200 border border-gray-200">Cancel</button>
                    <button type="submit" class="px-4 py-2 bg-yellow-100 text-yellow-800 rounded hover:bg-yellow-200 border border-yellow-200">Update</button>
                </div>
            </form>
        </div>
    </div>
