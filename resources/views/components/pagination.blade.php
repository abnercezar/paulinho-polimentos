...
@if ($paginator instanceof \Illuminate\Pagination\LengthAwarePaginator || $paginator instanceof \Illuminate\Pagination\Paginator)
    @if ($paginator->hasPages())
        <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-4 mt-6 rounded-b-lg">
            <div class="flex flex-1 justify-between sm:hidden">
                @if($paginator->onFirstPage())
                    <span class="mobile-btn relative inline-flex items-center rounded-lg border border-gray-300 bg-gray-100 px-4 py-3 text-sm font-medium text-gray-400 cursor-not-allowed">
                        ← Anterior
                    </span>
                @else
                    <a href="{{ $paginator->previousPageUrl() }}"
                       class="mobile-btn relative inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
                        ← Anterior
                    </a>
                @endif
                @if($paginator->hasMorePages())
                    <a href="{{ $paginator->nextPageUrl() }}"
                       class="mobile-btn relative ml-3 inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
                        Próximo →
                    </a>
                @else
                    <span class="mobile-btn relative ml-3 inline-flex items-center rounded-lg border border-gray-300 bg-gray-100 px-4 py-3 text-sm font-medium text-gray-400 cursor-not-allowed">
                        Próximo →
                    </span>
                @endif
            </div>
            <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        Exibindo
                        <span class="font-semibold">{{ ($paginator->currentPage() - 1) * $paginator->perPage() + 1 }}</span>
                        até
                        <span class="font-semibold">{{ min($paginator->currentPage() * $paginator->perPage(), $paginator->total()) }}</span>
                        de
                ...
                <div>
                    <nav aria-label="Paginação" class="isolate inline-flex -space-x-px rounded-lg shadow-sm">
                        {{-- Previous --}}
                        @if($paginator->onFirstPage())
